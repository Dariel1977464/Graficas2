...
public:
	TerrenoRR(int ancho, int alto, ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext)
	{
		...
		CargaParametros(L"terrain1.jpg", L"alturas3.jpg", 70.0f);
	}
	...
	bool CargaParametros(WCHAR* diffuseTex, WCHAR* heightTex, float tile)
	{
		...
		float blend_du = .5 / (float)(anchoTexTerr);
		float blend_dv = .5 / (float)(altoTexTerr);
		...
		for(int x = 0; x < altoTexTerr; x++)
		 {
			 for (int y = 0; y < anchoTexTerr; y++)
			 {
				 ...
				 vertcol[indiceArreglo].pos.y = vertices[indiceArreglo].pos.y = alturaData[x][y]/1.0;
		...
		d3dResult = D3DX11CreateShaderResourceViewFromFile( d3dDevice, L"Heightmap_CC1.jpg", 0, 0, &blendMap, 0 );
